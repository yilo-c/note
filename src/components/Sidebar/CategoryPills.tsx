...
const CategoryPills: React.FC = () => {
  ...
  const handleCatMouseDown = (e: React.MouseEvent, cat: typeof categories[0]) => {
    ...
    const onUp = (ev: MouseEvent) => {
      ...
      if (Math.sqrt(dx * dx + dy * dy) > 15) {
        ...
        if (isElectron) {
          ei.createFloatingWindow({ id, screenX: ev.clientX - 130, screenY: ev.clientY - 30, width: 260, height: 240 })
        }
      } else {
        // Click → filter
        setCategory(cat.id)
  ...
  const handleAdd = () => {
    ...
    if (!v) return
    addCategory({ id: uid(), label: v, icon: '馃搶', color: randomColor() })
    setNewLabel('')
    setShowAdd(false)
  }
        ...
        return (
          <div key={c.id} className="relative group">
            ...
            <button
              ...
              className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-[rgba(18,18,28,0.9)] border border-white/[0.08] flex items-center justify-center text-white/60 hover:text-red-400/70 text-[6px] opacity-0 group-hover:opacity-100 transition-opacity"
            >
        ...
        <AnimatePresence>
          {showAdd && (
            <motion.div
              ...
              className="absolute left-full ml-2 top-0 z-50 bg-[rgba(18,18,28,0.96)] border border-white/[0.08] rounded-xl p-3 shadow-2xl w-44"
              onMouseDown={e => e.stopPropagation()}
            >
              <div className="text-[10px] text-white/72 font-medium mb-2">新建分类</div>
              ...
              <div className="flex justify-end gap-1">
                <button onClick={() => setShowAdd(false)}
                  className="text-[9px] px-2 py-1 rounded text-white/65 hover:text-white/60 transition-colors">取消</button>
                <button onClick={handleAdd}
                  className="text-[9px] px-2 py-1 rounded bg-fluent-blue/15 text-fluent-blue/60 hover:bg-fluent-blue/20 transition-colors">添加</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
