...
│   ├── shared/                      # 纯逻辑包 — 两端共用
│   │   ├── src/
│   │   │   ├── types/               # 数据模型（TodoItem, FloatingNote, UserProfile 等）
│   │   │   ├── store/               # Zustand 核心状态（无平台依赖）
│   │   │   │   ├── useStore.ts      # 主 store（~800 行，含 persist + AI 订阅）
│   │   │   │   ├── helpers.ts       # getDescendantIds（子任务递归）
│   │   │   │   └── storage.ts       # PlatformAdapter 抽象
│   │   │   ├── utils/               # 工具函数
│   │   │   │   ├── ai/              # AIProvider 接口 + LocalAIProvider
│   │   │   │   ├── astrology/       # 命理引擎（黄历/八字/奇门/择日）
│   │   │   │   └── helpers.ts       # uid(), defaultTodos 等
...
│   │   │   ├── App.tsx              # 桌面入口
│   │   │   ├── components/          # 桌面 UI 组件
│   │   │   │   ├── Todo/            # 待办（CalendarView, KanbanBoard, QimenPanel 等）
│   │   │   │   ├── FloatingNote/    # 浮动便签
│   │   │   │   ├── Common/          # 通用组件
│   │   │   │   └── Search/          # 全局搜索
│   │   │   └── adapters/            # ElectronStorageAdapter
│   │   ├── vite.config.ts
│   │   └── tailwind.config.js
│   │
│   └── mobile/                      [已废弃 — PWA 限制过多。新手机端为独立 Capacitor 项目]
│                                    → D:\h\3.25\便签应用\思忆便签手机端app\
```

### 手机端项目（独立）

手机端已从 monorepo 中移出，作为独立 Capacitor 项目存在。它依赖 `@desk-notes/shared` 通过 `file:` 引用。详见该项目的 [CLAUDE.md](../../../思忆便签手机端app/CLAUDE.md)。
